...
- **The Streamlit stats page (`/stats`) and the audio livestream are installed but not enabled.** Each keeps a large resident process alive; on 512MB there isn't room. Turn either on from *Tools → Services* if you want them.
